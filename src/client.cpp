...
i32 main() {
    ...
    const ifstream inp_fs("tests/testin.txt");
    const ofstream out_fs("tests/testout.txt", ios_base::openmode::_S_out);
    // flags mode
    // const file_desc fd_out = open("tests/testout.txt", O_TRUNC | O_CREAT | O_RDWR, 0666);
    // const u32 res = chmod("tests/testout.txt", 0666);
    array<u8, BUF_MAX> buffer{};
    inp_fs.read(&buffer[0], BUF_MAX);
    out_fs.write(&buffer[0], BUF_MAX);
